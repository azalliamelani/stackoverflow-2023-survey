...
@st.cache
def load_data():
    # Define the URL of the GitHub repository where the .7z file is stored
    github_url = "https://github.com/azalliamelani/stackoverflow-2023-survey/raw/main/data/survey_results_public.7z"
    
    # Make a request to download the .7z file
    response = requests.get(github_url)
    
    # Extract the contents of the .7z file
    with py7zr.SevenZipFile(BytesIO(response.content), mode='r') as z:
        # Assume the .7z file contains a single CSV file
        csv_filename = z.getnames()[0]
...
st.sidebar.image("D:/sofanalysis/dashboard/logo.png", use_column_width=True)
...
st.subheader("Top 10 Respondent Based On Country Distribution")
...
fig_lang = px.bar(
    ...
    orientation='h',
    color_discrete_sequence=['#69B4FF'] * len(language_counts)  # Set the color to #69B4FF
)
...
fig_db = px.bar(
    ...
    orientation='h',
    color_discrete_sequence=['#69B4FF'] * len(database_counts)  # Set the color to #69B4FF
)
...
fig_platform = px.bar(
    ...
    orientation='h',
    color_discrete_sequence=['#69B4FF'] * len(platform_counts)  # Set the color to #69B4FF
)
...
fig_webframe = px.bar(
    ...
    orientation='h',
    color_discrete_sequence=['#69B4FF'] * len(webframe_counts)  # Set the color to #69B4FF
)
...
fig_collabtool = px.bar(
    ...
    orientation='h',
    color_discrete_sequence=['#69B4FF'] * len(collabtool_counts)  # Set the color to #69B4FF
)
